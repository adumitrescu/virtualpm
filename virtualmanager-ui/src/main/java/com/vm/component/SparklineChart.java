...
public class SparklineChart<T> extends VerticalLayout {

    public SparklineChart(final String name, final String unit,
            final String prefix, final Color color, final List<T> list, final Object[] values, 
...
//                min, max);
        ...
        addComponent(buildSparkline(list, color));

        Label highLow = new Label("High <b>" +max
                + "</b> &nbsp;&nbsp;&nbsp; Low <b>"
                + min + "</b>", ContentMode.HTML);
    ...
    private Component buildSparkline(final List<T> values, final Color color) {
        ...
        for (int i = 0; i < values.size(); i++) {
            DataSeriesItem item = new DataSeriesItem("", (Number) values.get(i));
            series.add(item);
        }
